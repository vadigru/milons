<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">

  <link rel="stylesheet" href="../css/style.css">
  <link rel="stylesheet" href="../css/fontawesome-all.css">
  <link rel="icon" type="image/png" href="../img/milons_icon.png">
  <title>Борцовский клуб "Милонс"</title>
</head>

<body id="group_1">
  <div class="wrapper">

    <?php include "blocks/header.php"; ?>

    <main>
     <div class="schedule">
       <div class="schedule_content">
         <table>
          <thead>
            <tr>
              <td colspan="2"> <h2>Расписание тренировок для группы 1</h2></td>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>Понедельник</td>
              <td>16:00 - 18:00</td>
            </tr>
            <tr>
              <td>Вторник</td>
              <td>16:00 - 18:00</td>
            </tr>
            <tr>
              <td>Среда</td>
              <td>16:00 - 18:00</td>
            </tr>
            <tr>
              <td>Четверг</td>
              <td>16:00 - 18:00</td>
            </tr>
            <tr>
              <td>Пятница</td>
              <td  id="group_2">16:00 - 18:00</td>
            </tr>
            <tr>
                <td>Суббота</td>
                <td>Вечеринка</td>
              </tr>
              <tr>
                <td>Воскресенье</td>
                <td>Отдыхаем</td>
              </tr>
          </tbody>
          <tfoot></tfoot>
         </table>
       </div>
     </div>
     <div class="news_divider"></div>
     <div class="schedule">
        <div class="schedule_content">
          <table>
           <thead>
             <tr>
               <td colspan="2"> <h2>Расписание тренировок для группы 2</h></td>
             </tr>
           </thead>
           <tbody>
             <tr>
               <td>Понедельник</td>
               <td>16:00 - 18:00</td>
             </tr>
             <tr>
               <td>Вторник</td>
               <td>16:00 - 18:00</td>
             </tr>
             <tr>
               <td>Среда</td>
               <td>16:00 - 18:00</td>
             </tr>
             <tr>
               <td>Четверг</td>
               <td>16:00 - 18:00</td>
             </tr>
             <tr>
               <td>Пятница</td>
               <td>16:00 - 18:00</td>
             </tr>
             <tr>
               <td>Суббота</td>
               <td>Вечеринка</td>
             </tr>
             <tr>
               <td>Воскресенье</td>
               <td>Отдыхаем</td>
             </tr>
           </tbody>
           <tfoot></tfoot>
          </table>
        </div>
      </div>
      <div class="news_divider"></div>
      <div class="schedule">
          <div class="schedule_content">
            <table id="group_3">
             <thead>
               <tr>
                 <td colspan="2"> <h2>Расписание тренировок для группы 3</h2></td>
               </tr>
             </thead>
             <tbody>
               <tr>
                 <td>Понедельник</td>
                 <td>16:00 - 18:00</td>
               </tr>
               <tr>
                 <td>Вторник</td>
                 <td>16:00 - 18:00</td>
               </tr>
               <tr>
                 <td>Среда</td>
                 <td>16:00 - 18:00</td>
               </tr>
               <tr>
                 <td>Четверг</td>
                 <td>16:00 - 18:00</td>
               </tr>
               <tr>
                 <td>Пятница</td>
                 <td>16:00 - 18:00</td>
               </tr>
               <tr>
                  <td>Суббота</td>
                  <td>Вечеринка</td>
                </tr>
                <tr>
                  <td>Воскресенье</td>
                  <td>Отдыхаем</td>
                </tr>
             </tbody>
             <tfoot></tfoot>
            </table>
          </div>
        </div>

    </main>

    <?php include "blocks/footer.php"; ?>

  </div>
  <script src="../js/script.js"></script>
  <script src="../js/pure-swipe.min.js"></script>
</body>

</html>
